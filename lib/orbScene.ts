...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  // Text sprites share a small pool of pre-rendered textures/materials
  // (keyed by snippet + color variant) instead of each sprite getting its
  // own canvas + texture + material. With thousands of sprites, unique
  // per-instance textures meant thousands of GPU texture uploads and
  // material state changes every draw — this cuts that to a few dozen.
  const TEXT_VARIANTS = 4;
  const textMaterialCache = new Map<string, THREE.SpriteMaterial>();

  function getTextMaterial(text: string, variant: number): THREE.SpriteMaterial {
    ...
    const alpha = 0.35 + (variant / (TEXT_VARIANTS - 1)) * 0.55;
    ctx.fillStyle = `rgba(255, ${(130 + variant * 20) | 0}, ${(20 + variant * 8) | 0}, ${alpha})`;
    ...
    textMaterialCache.set(key, mat);
    return mat;
  }

  function makeTextSprite(size = 0.08) {
    const text = codeSnippets[Math.floor(Math.random() * codeSnippets.length)];
    const variant = Math.floor(Math.random() * TEXT_VARIANTS);
    const s = new THREE.Sprite(getTextMaterial(text, variant));
  ...
  function scatterText(count: number, sizeFn: () => number, rFn: () => number, speedScale: [number, number]) {
    ...
    for (let i = 0; i < count; i++) {
      const sp = makeTextSprite(sizeFn());
  ...
  // ═══════════════════════════════════════════════
  // Instanced per geometry type — 6 draw calls total instead of one mesh
  // (and one material) per satellite. Rotation/orbit motion is still
  // per-instance, just written into a shared matrix buffer each frame.
  const debrisGeos = [
  ...
  const debrisMeshes: THREE.InstancedMesh[] = [];
  const debrisData: DebrisOrbit[][] = [];
  for (const geo of debrisGeos) {
    ...
    const inst = new THREE.InstancedMesh(geo, mat, DEBRIS_PER_GEO);
    const data: DebrisOrbit[] = [];
    for (let i = 0; i < DEBRIS_PER_GEO; i++) {
      data.push({
      ...
      inst.setColorAt(i, new THREE.Color(Math.random() > 0.7 ? C_BRIGHT : C_MID));
    }
    if (inst.instanceColor) inst.instanceColor.needsUpdate = true;
    debrisData.push(data);
    debrisMeshes.push(inst);
    orbGroup.add(inst);
  }
  ...
  function animate() {
    ...
    debrisMeshes.forEach((inst, gi) => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        debrisDummy.rotation.set(u.rotX, 0, u.rotZ);
        debrisDummy.updateMatrix();
        inst.setMatrixAt(i, debrisDummy.matrix);
      }
